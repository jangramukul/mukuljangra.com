...
**Young generation (nursery space):** Newly allocated objects land here. This region is small and collected frequently using a semi-space copying algorithm — it copies live objects to the other half of the space and reclaims the dead ones in bulk. This is fast because most young objects die quickly (the "generational hypothesis"), so only a small fraction needs copying. Young generation GC pauses are typically under 2ms on modern devices.
...
**Large object space:** Objects larger than 12KB (primarily Bitmaps and large arrays) go directly here, bypassing the young generation entirely. These are collected with the old generation. This is why large Bitmap leaks are especially painful — they never get the benefit of fast nursery collection.
...
Android's `lmkd` (Low Memory Killer Daemon) assigns every process an `oom_adj_score` based on its importance. The buckets, roughly, are: foreground processes (the Activity the user is currently interacting with) get the lowest score and are killed last. Visible processes (like an Activity behind a transparent dialog) are next. Service processes (running a foreground Service or active work) sit in the middle. Finally, cached processes — apps the user has navigated away from — have the highest score and are killed first. Within each bucket, the system kills the process using the most memory first.
...
I think about the Low Memory Killer as the final reason why memory discipline matters on Android. You're not just competing with your own heap limit. You're competing with every other app on the device for physical RAM, and the OS is actively arbitrating who gets to stay alive. An app that leaks 50MB might not OOM, but it will get killed in the background more aggressively, lose its saved state more often, and deliver a worse experience on low-RAM devices — even if it technically never crashes.
...
Android Studio's Memory Profiler is the primary tool for detecting leaks in development. The approach I use is systematic: establish a baseline heap size after a forced GC, trigger the suspected leak by repeating the action 5-10 times (like navigating to a screen and back), force GC again, and capture a heap dump. If your heap has grown significantly and stays grown after GC, you have a leak. In the heap dump, sort by **Retained Size** and look for multiple instances of Activities or Fragments that should only have one instance alive. The reference chain from any leaked instance tells you exactly which field is preventing collection.

One honest limitation: **debug builds allocate differently than release builds.** Debug builds disable R8 optimizations and add debugging metadata, creating allocation patterns you won't see in production. This is where LeakCanary becomes essential. In development, it watches for Activity and Fragment instances that should have been collected, forces GC, and analyzes the reference chain automatically. But the part that changed how our team works is `leakcanary-android-release` — the production variant. We integrated it with our analytics pipeline at 5% sampling and discovered that 15% of our OOM crashes were caused by a single `DialogFragment` callback leak. The fix was three lines of code, and OOM crashes dropped by 12% in the next release.
