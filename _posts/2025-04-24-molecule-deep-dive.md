...
Think of it this way. You want to say: "if the user exists and their email isn't verified, show a banner." That's one `if` statement. But in the reactive world, you first have to set up the flow for the user, set up the flow for the email status, combine them, handle initial values for both, convert to a StateFlow, configure sharing... you've written 30 lines of plumbing for a one-line decision.
...
Here's an analogy that makes this click. Think of Compose like a car engine and a car body. The engine (compiler + runtime) provides the power — it tracks changes, knows when something is stale, and recalculates efficiently. The car body (Compose UI) is just one thing you can bolt onto that engine. Molecule takes that same engine and bolts on a completely different body — one that produces `StateFlow` values instead of pixels on a screen.

> **💡 The "aha" moment:** Compose's real superpower isn't drawing UI. It's tracking state changes and recomputing only what changed. Molecule takes that superpower and uses it for pure state management — no UI nodes, no rendering, just state production.

Molecule exploits this separation. It takes the Compose compiler and runtime — the state tracking, the recomposition system, `remember`, `LaunchedEffect`, `derivedStateOf`, all of it — and uses them purely for state management. As the Molecule README puts it, it "just" glues Compose's state management to `kotlinx.coroutines` flows so that it can be used without the node tree.
...
Look at that. No `combine`. No `stateIn`. No `SharingStarted`. Just... collect the values and use them. It reads like regular Kotlin.

Now compare this with the traditional `Flow`-based approach:
...
With two flows, the traditional approach looks manageable. Add 5-6 more sources and the `combine` nesting becomes unreadable. The Molecule version stays flat and imperative regardless of how many sources you add — you just add more `collectAsState` calls and use the values directly in Kotlin control flow. It's like the difference between giving someone step-by-step origami instructions over the phone versus handing them the finished shape and saying "look, it's a crane." The Molecule version lets you see the whole picture at once.
...
Notice the event sink pattern — the last lambda in the `PaymentUiState` constructor. The UI sends events back to the presenter through this lambda, which is included directly in the state object. The presenter handles each event by mutating its local compose state, which triggers recomposition and produces a new state.

This is unidirectional data flow, but without any of the `MutableStateFlow`, `combine`, or `stateIn` plumbing. The business logic reads like imperative Kotlin. You might be wondering — "wait, if the event handler just mutates local state, how does the UI get the new state?" That's the beauty of it. Mutating compose state triggers recomposition, which re-runs the function, which returns a new `PaymentUiState` with the updated values. The Compose runtime does the heavy lifting for you.
...
> **⚡ Quick check:** If you opened your most complex ViewModel right now, what percentage of the code is actual business logic vs. reactive pipeline wiring? If it's less than 50% business logic, Molecule might be worth a serious look.
...
> **🧠 Think about it:** Every new abstraction trades one kind of complexity for another. Molecule trades reactive pipeline complexity for recomposition complexity. The question isn't "which has zero complexity?" — it's "which complexity matches how your brain already works?"
