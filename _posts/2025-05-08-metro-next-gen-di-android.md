...
Every step forward solved a real problem but carried the weight of the previous generation's constraints. We were writing Kotlin-first apps powered by a Java-first DI framework, processed by a compatibility layer, generating code in a language we weren't writing anymore. It's like renovating a house one room at a time, but the foundation is still from the 1960s — every new room looks modern, but the plumbing underneath keeps reminding you how old everything really is.

It worked. But it felt like we were holding the ecosystem together with duct tape.
...
Now here's where it gets interesting. Here's the thing that reframed how I think about DI tooling: **Metro doesn't process your code and generate new source files. It operates inside the Kotlin compiler itself.**

What does that actually mean? Think of it like this. Dagger is like a translator standing between two people — it reads your Kotlin code, writes new Java files, and then the Java compiler processes those generated files in a separate pass. Metro is more like a bilingual person who just understands both sides natively. It uses FIR for error reporting and diagnostics, and both FIR and IR for code generation. It generates code directly into the compiler's intermediate representation, skipping the source-generation round-trip entirely.

This is why Metro can do things that source-generation tools physically cannot — like reading private declarations, using default parameter values as optional dependencies, and injecting into private properties. It's not limited by what's visible from outside a file. It's inside the compiler. Sounds weird, right? But that single architectural choice is what makes everything else possible.
...
> **💡 The "aha" moment:** Dagger can't access your `private` members or default values because it's a guest looking at your code from the outside. Metro sees everything because it lives inside the Kotlin compiler — it's not a guest, it's a roommate.
...
That's it. Seriously, that's the whole setup.

The plugin handles adding Metro's runtime dependencies and all the compiler plugin wiring. No annotation processor configuration, no kapt block, no KSP setup. If you've ever spent an afternoon debugging a Dagger/kapt setup in a multi-module project, you'll appreciate how refreshing this is. If you want IDE support for Metro's FIR diagnostics and generated declarations in the editor, you need K2 mode enabled and the `kotlin.k2.only.bundled.compiler.plugins.enabled` registry flag set to `false` in IntelliJ. It's not required for compilation, but it's nice to have for error highlighting.
...
This is the pattern you reach for when some dependencies come from the graph (like `PaymentGateway`) and others come from the call site at runtime (like the payment `amount`). The factory gives you a clean way to mix both without manual wiring.

Metro also supports member injection for classes you can't constructor-inject, like Android Activities. Unlike Dagger, injected members can be `private` — you expose a `fun inject(target: MyActivity)` function on the graph, and Metro handles the rest. No more `@set:Inject lateinit var` with `internal` visibility just to make the annotation processor happy.
...
Can you guess what's happening under the hood? Metro sees that `AppGraph` extends `NetworkProviders`, so any `@Provides` functions in `NetworkProviders` become part of the graph automatically. No `@Module`, no `@InstallIn`, just Kotlin interfaces doing what interfaces do.
...
Metro validates the entire dependency graph at compile time using Tarjan's algorithm and topological sort. Missing bindings, circular dependencies, and scope mismatches are all caught during compilation with detailed error messages that tell you exactly what's wrong and how to fix it. If you've ever stared at a Dagger error message that reads like a stack trace from another dimension, you'll appreciate Metro's error reporting. It tells you what's missing, where it's expected, and how to fix it.
...
Every call to `appGraph.authManager` returns the same instance. Declaring a `scope` on the graph is functionally equivalent to annotating the graph itself with `@SingleIn` for that scope. Metro enforces that scoped bindings match their graph's scope — a binding scoped to `UserScope` in an `AppScope` graph is a compile-time error. Not a runtime crash. Not a silent bug. A compile-time error. That's the kind of safety net you want.

For parent-child graph hierarchies, Metro uses `@GraphExtension` — similar to Dagger's subcomponents. Think of it like inheritance: the child graph inherits all the bindings from its parent and can add its own on top:
...
Imagine you have 50 feature modules. Without aggregation, your app module needs to manually list every single binding from every single module. Add a new feature? Update the app module. Remove one? Update the app module. It's like maintaining a guest list for a party where every guest has to be manually invited — and the party is every day.

With Metro's aggregation, each feature module just declares "I belong to this scope" and Metro handles the rest:
...
Yeah, read those numbers again. 17.5 seconds vs 119.6 seconds. That's not a marginal improvement — that's "go get coffee vs. stay at your desk" territory.
...
> **🔥 Real talk:** Real-world results confirm the synthetic benchmarks. Cash App reported ~59% faster incremental builds and 16% faster clean builds after migrating their 1,500-module codebase. Freeletics saw 40-55% faster ABI changes across 551 modules. BandLab measured 55% faster incremental builds on their 929-module project. These are production numbers from teams that shipped the migration — not theoretical projections.
...
But the migration isn't friction-free. Cash App had to fix nullability mismatches that Dagger's Java heritage had been silently ignoring, convert `@Component.Builder` to `@Component.Factory` (Metro has no builder equivalent), remove direct calls to `@Provides` methods from test code, and untangle some `@MergeModule` patterns. Most of these were tech debt that should have been cleaned up anyway — Metro's stricter validation just exposed it. I think that's actually a good sign for the framework's design philosophy. A tool that forces you to clean up long-standing messes is doing you a favor, even if it doesn't feel like it at the time.

> **🧠 Think about it:** If your DI migration surfaces a bunch of hidden nullability bugs and architectural shortcuts, was the old framework "working" — or was it just not telling you about the problems?
