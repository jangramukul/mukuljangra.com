...
Early in my career, I shipped an app that stored API keys in `BuildConfig` fields and user tokens in plain `SharedPreferences`. I thought `BuildConfig` was safe because it was "compiled" and `SharedPreferences` was fine because the files were in the app's private directory. Both assumptions were wrong. A motivated user with a rooted device could decompile the APK in under a minute and extract every `BuildConfig` constant. The SharedPreferences XML files sat on disk in plain text, readable by any process with root access.

That was my "oh no" moment.

I learned the hard way that **Android's sandbox protects you from other apps, but it doesn't protect you from the device owner.** Think of it like a hotel room. The lock on the door keeps other guests out, but the person inside the room can see everything. And a determined hotel manager with a master key? They're getting in no matter what.

That experience reframed how I think about mobile security. You're not building a fortress — you're building layers of defense that make attacks progressively harder. It's like those anti-theft systems in retail stores. No single tag, camera, or locked case is unbeatable. But stack enough of them together, and a shoplifter decides it's not worth the effort. That's exactly the mindset you want. No single measure is bulletproof, but the combination of encrypted storage, certificate pinning, proper key management, biometric gates, and code obfuscation raises the cost of an attack high enough that most adversaries move on. Here's how I approach Android security now, from the most critical layer outward.

## The Android Keystore — Where Secrets Belong

Imagine you have a safe deposit box at a bank. You can put things in, take things out, and use what's inside — but you can never photocopy the key itself. Even the bank employees can't duplicate it. That's essentially what the Android Keystore does for your cryptographic keys.
...
This is fundamentally different from storing keys in your code or in a file. A key in the Keystore can't be extracted because the hardware won't export it — the attacker can use the key if they have your app's UID, but they can't copy it to another device or read the raw key material. The key stays trapped inside the hardware. You send data in, encrypted data comes out. That's it.
...
But `EncryptedSharedPreferences` only covers key-value data. What about structured data?

For structured data in **Room databases**, you need **SQLCipher**. Think of it like putting your entire filing cabinet inside a locked room, rather than just locking individual drawers. SQLCipher encrypts the entire SQLite database file transparently. You swap `Room.databaseBuilder` for `SupportFactory` from the `net.qlite:android-database-sqlcipher` library and pass a passphrase — ideally one derived from a Keystore-backed key, not a hardcoded string. I use this in any app that caches user data locally: medical records, financial transactions, chat messages. The performance overhead is roughly 5-15% on queries depending on dataset size, which is acceptable for most apps.
...
> **🧠 Think about it:** If someone steals a user's device and roots it, which of your app's data would they be able to read right now? Auth tokens? Cached messages? Database contents? If any of those answers make you uncomfortable, you know which layer to add next.

## Certificate Pinning and Network Security

Alright, your data on the device is encrypted. But what about when it's traveling over the network?

HTTPS encrypts traffic between your app and the server, but here's the catch — it trusts any certificate authority in the device's trust store. A compromised CA or an enterprise proxy injecting its own certificate can perform a man-in-the-middle attack. It's like sending a sealed letter through a courier service. The letter is sealed, sure, but you're trusting that the courier service is legit. What if someone set up a fake courier service that opens your letter, reads it, reseals it, and delivers it? That's a MITM attack. Certificate pinning is you saying "I only trust THIS specific courier, and here's their photo ID."

Certificate pinning restricts which certificates your app accepts, so even if the device trusts a rogue CA, your app won't.
...
Here's a question for you: when a user taps "Confirm Payment" in your app, how do you know it's actually the user and not someone who picked up their unlocked phone?

The **BiometricPrompt** API (from `androidx.biometric`) is how you gate high-value operations behind fingerprint, face, or iris authentication. I use it for payment confirmations, viewing sensitive health data, and re-authenticating after the app has been backgrounded for too long. But here's the key thing most tutorials skip: BiometricPrompt alone only confirms "a biometric matched." That's it. Just a boolean. For real security, you need to pair it with a **CryptoObject**.

Why? Because without a CryptoObject, your biometric check is just a UI gate — like a bouncer who checks IDs at the door but leaves the back door wide open.

A `CryptoObject` wraps a `Cipher`, `Signature`, or `Mac` instance that's tied to a Keystore key with `setUserAuthenticationRequired(true)`. When the user authenticates, the TEE unlocks that specific key for one operation. This means even if an attacker somehow bypasses the biometric UI, they can't use the underlying cryptographic key without the actual biometric match in hardware. The bouncer and the lock are now the same thing.
...
Wait, what? Yeah. You just gave a mini browser engine the ability to run code inside your app's sandbox. That's like inviting a stranger into your house and handing them the keys to every room.
...
> **🔥 Real talk:** I've reviewed codebases where developers called `handler.proceed()` in `onReceivedSslError` just to "fix" SSL errors during development and forgot to remove it before shipping. That one line turns your WebView into a wide-open attack surface. Treat it like a `TODO` that should never exist in release code.

## Play Integrity API — Verifying Device and App Trust

So far, we've been defending the app from attacks on the device. But what if the device itself is the problem? What if someone is running a modified version of your APK on an emulator, or a bot is hammering your server pretending to be a real user?
...
The API returns a signed integrity verdict containing a **device integrity** field (MEETS_DEVICE_INTEGRITY, MEETS_BASIC_INTEGRITY, or empty for compromised devices), an **app integrity** field (confirming the APK hash matches Play), and an **account licensing** field. Here's the key architectural point, and this is critical: never evaluate the verdict on-device. Always send the integrity token to your server, have the server call Google's API to decode it, and make the trust decision server-side. If you evaluate on-device, an attacker can just patch your verification code. It's like asking a suspect to grade their own lie detector test.
...
I think of root detection as a speed bump, not a wall. It won't stop a determined driver, but it'll slow down most of them and it'll definitely stop the cyclists. In a banking app I worked on, we checked for root at startup and flagged the session server-side, applying stricter rate limits and transaction caps rather than blocking outright. The real enforcement happened server-side based on the Play Integrity verdict combined with root detection signals.

> **💡 The "aha" moment:** Root detection on-device can always be bypassed — it's code running on the attacker's machine. The real value is using it as one signal among many on your server, where the attacker can't tamper with your logic.

## R8 Obfuscation and Beyond — Making Reverse Engineering Expensive

Imagine someone decompiles your APK and gets back nearly readable source code — your class names, method names, string constants, the whole thing. That's what happens without R8.
...
Put differently: R8 turns your source code from an open book into a puzzle. It's still solvable, but it'll cost hours instead of minutes.
...
Think of it like doors in a building. Every `<intent-filter>` is a door. Before Android 12, adding an intent-filter was like installing a door that defaulted to unlocked. Now you have to explicitly choose locked or unlocked — but you still need to think about which doors should be open at all.
...
For secure logout, clear more than just your own storage. Invalidate the tokens server-side first, then clear `EncryptedSharedPreferences`, clear any Room database caches, cancel all pending work with WorkManager, and clear the WebView cookie jar if you use one.

I've seen apps that "log out" by just clearing the token locally — the old refresh token is still valid on the server, and if someone extracts it from a backup or compromised device, they can generate new access tokens indefinitely. That's not a logout. That's hiding the key under the doormat and calling it "locked."
...
> **⚡ Quick check:** Does your app's logout flow invalidate tokens on the server before clearing local storage? If not, a compromised refresh token can generate new access tokens forever — even after the user "logs out."
