...
Your users are in subways, elevators, and that one corner of the office where Wi-Fi goes to die. Offline-first is one of the most practical system design topics because it forces you to rethink where truth lives in your app.

#### What does "offline-first" actually mean? How is it different from just caching?

Here's the thing — there are levels to this. A basic cache is like keeping a photocopy of a document. When the original is unavailable, you can still read your copy, but you can't write anything new on it. That's read-only stale data.

Offline-first flips the whole model. The local database *is* the document. Every read hits the local DB. Every write goes to the local DB first. The network? That's just the courier service that eventually delivers your changes to the server and brings back everyone else's. The server is where data eventually converges, not where it lives.

#### What are the functional requirements for an offline-first app?

The user should be able to do full CRUD while offline — create, read, update, delete — without any network connection. All changes get persisted locally and queued for sync. When the device reconnects, the app pushes pending local changes to the server and pulls remote changes. No manual "sync now" button needed.

The app also needs to handle conflicts — what happens when the same record was modified both locally and on the server while you were offline.
...
Think of it like a post office. The local database (Room) is the single source of truth — that's the mailbox everyone in your house checks. The UI layer observes Room via Flow, so it always knows what's in the mailbox. The sync engine is the postal worker — it picks up outgoing mail (local changes) and delivers incoming mail (remote changes). A pending operations queue holds your outgoing letters. WorkManager makes sure the postal worker shows up whenever the road (network) is clear.

The data flow is: UI writes to local DB, local DB notifies UI via Flow, sync engine pushes to server in the background. For reads: UI reads from local DB, sync engine pulls remote changes into local DB, UI gets notified automatically.
...
> **🧠 Think about it:** If both the local DB and the server have a version of the same record, and they disagree — who wins? What information do you need to even detect that disagreement?
...
Think of this like a transaction log at a bank. Every local write gets recorded as an operation — what type (create, update, delete), which entity, the payload, a timestamp, and a retry count. You process the queue in FIFO order to maintain causality — a create must sync before an update to the same entity.
...
Here's the key design decision — every user action, *even when online*, goes through this queue. This keeps the data flow consistent regardless of network state. One path, always.

#### How would you design the API for delta sync?

Full sync downloads everything on every cycle, which is like re-downloading your entire email inbox every time you check for new messages. Delta sync only transfers what changed. The server accepts a `syncToken` (or `since` timestamp) and returns only records created, updated, or deleted after that point, along with a new sync token.

The client stores the last sync token locally. On each sync cycle, it sends the token and receives only the delta — insert new records, update modified ones, soft-delete removed ones. If the token is too old and the server has purged its change log, fall back to a full sync. The server response should include a flag like `fullSyncRequired: true` for this case.

#### How do you detect conflicts during sync?

Compare the local version with the server version relative to the last synced version. If only the local version is ahead of `lastSyncedVersion`, push local changes. If only the server version is ahead, accept the server data. If both are ahead — yeah, you have a conflict.

For timestamp-based detection, the logic is similar — compare `localUpdatedAt` and `serverUpdatedAt` against `lastSyncedAt`. But the version-based approach is more reliable because timestamps depend on device clocks, which can drift. Server-assigned version numbers are monotonic and don't have the clock skew problem.
...
> **🧠 Think about it:** You're building a collaborative note-taking app. One user edits the title on their phone while another edits the body on their tablet — both offline. When they reconnect, is this actually a conflict?

#### How would you implement field-level merge?

Plot twist: it doesn't have to be. Track which fields changed since the last sync using per-field dirty flags. During sync, if the client changed the title and the server changed the body, take the client's title and the server's body — no conflict. Only if both changed the *same* field do you fall back to LWW or manual resolution for just that field.
...
This reduces visible conflicts dramatically. In a note-taking app, one device changing the title and another changing the body is super common — without field-level tracking, that's a false conflict every time.
...
Yeah, this trips up everyone. Without compaction, creating and deleting a note offline would result in a create request followed by a delete request — two completely wasted round trips for something that nets to zero.

#### How do you use WorkManager for background sync?

WorkManager is the right tool because it guarantees execution even if the app is killed — it's like a reminder that keeps nagging the OS until the job gets done. It respects Doze mode and battery optimization. Schedule a one-time sync worker with a network constraint — it only runs when connectivity is available.
...
One gotcha — `NET_CAPABILITY_INTERNET` doesn't guarantee the server is reachable. You can be connected to Wi-Fi where the router has no upstream connection. It's like seeing a road on the map but not knowing there's a bridge out. A robust app should handle failed network requests gracefully rather than trusting the connectivity flag alone.

#### How do you handle schema migrations when users have unsynced data?

Now here's where it gets interesting. This is harder than regular migrations because the user might have pending operations in the old schema when the app updates. Room migrations handle the local DB schema change, but the API contract also changes. If the server adds a new required field, old clients syncing without it will fail.
...
Apply changes to the local DB immediately and let the UI update via Flow. The user sees the result instantly — it feels like the app is lightning fast. In the background, the sync engine pushes the change to the server. If the server rejects it (validation error, conflict, permission denied), revert the local state and show an error.
...
> **🧠 Think about it:** What happens if the user makes three more edits while the first one is still syncing — and then that first sync fails? How do you roll back without losing the subsequent edits?
